...
plot.shiny.km <- function(time, death, x, title = "", ids = NULL, 
                          ...
                          no.plot = FALSE, optimal.cut = FALSE, qtle = 0.5, LEGEND = TRUE, lwd = 0.5, xmax = NULL, txt.size = 12, ...) {
  ...
  # print('saving km data')
  # save(km.data, qtle, optimal.cut, x, file = 'look.RData')
  
    ## settings for median cutoff ##
  cut = quantile(x, qtle)
  p.adj = NULL

  upper = paste0("upper ", 100-qtle*100, "%")
  lower = paste0("lower ", qtle*100, "%")
#  upper = "upper 50%"; lower = "lower 50%"
  ...
  km.group = km.group+theme(axis.text=element_text(size=txt.size))
  #    theme(plot.title = element_text(vjust = 0.5, colour = "black"))
 
  if (!is.null(xmax)) { 
    plot(km.group+ xlim(0,xmax))
  } else {
    plot(km.group)
  }
